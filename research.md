...
To illustrate the power of our **Latent Diffusion for Full Protein Generation** framework, below are two short clips generated from our trained model.  The first shows raw molecular dynamics (MD) snapshots of the human PKC‑δ C1 domain followed by frames generated by LD‑FPG.  You can observe how the model faithfully captures backbone flexibility, helix breathing and loop motions.  The second video shows the protein 3dAN. Once trained, LD‑FPG can generate thousands of physically realistic, all‑atom conformations in seconds — ideal for tasks such as ensemble docking, AI‑augmented scoring and any workflow that requires a rich representation of protein motion.
