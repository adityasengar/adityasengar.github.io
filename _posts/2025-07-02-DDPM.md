...
They used a **Variance Preserving (VP)** approach. At each of the `T` discrete timesteps, they added a small amount of noise according to a schedule of variances `βt`. The noised image at step `t` is created from the image at `t-1` like this:

$$
x_t = \sqrt{1 - \beta_t} \cdot x_{t-1} + \sqrt{\beta_t} \cdot \epsilon_{t-1} \quad \text{where} \quad \epsilon \sim \mathcal{N}(0, I)
$$

Here, `βt` is small, so we're mostly keeping the old image and adding just a bit of noise. The real magic of the forward process is that, thanks to the properties of Gaussian noise, we don't have to apply this iteratively. We can calculate `xt` directly from the original image `x₀` for any `t`. This is a crucial **reparameterization** of the process:
...
Instead of predicting the mean of the previous image $\mu_\theta(x_t, t)$ directly, the authors of DDPM found it much more effective to reparameterize the objective and train the model to predict the **noise** (`ϵ`) that was added. The reasoning is that since `xt` is just a linear combination of `x₀` and `ϵ`, predicting `ϵ` is mathematically equivalent to predicting `x₀`, but leads to more stable training.

This leads to a beautifully simple and elegant loss function—a straightforward Mean Squared Error between the true noise and the predicted noise:
...
| :--- | :--- | :--- |
| **Defining the Noise Level** | Uses a **discrete schedule**. You set `beta_start`, `beta_end`, and a scheduler (`linear`, `cosine`) to define `βt` for `T` steps. | Uses a **continuous function `σ(t)`**. The noise level `σ` is the primary concept. `beta` is just a consequence of how `σ` changes over time. |
| **Time & Steps** | A fixed, large number of **timesteps `T`** (e.g., 1000) is defined for both training and sampling. | **Decouples training and sampling.** Training happens over a continuous `σ`. For sampling, you choose a much smaller number of **evaluation steps `N`** (e.g., 40). |
| **Controlling Step Spacing** | The choice of scheduler (`linear`, `cosine`) is your only, indirect control. You can't easily concentrate steps in a specific region. | Introduces a direct control parameter, **`ρ` (rho)**. This explicitly controls the distribution of the `N` sampling steps across the noise levels (`ρ=7` is a common default). |
...
As shown in the table, the EDM way is to throw out discrete timesteps `t` in favor of a continuous noise level `σ`. This is a more flexible abstraction that allows for the creation of new, more intelligent schedules. Using the `ρ` parameter, you can now concentrate the sampling steps where they are most needed—typically at lower noise levels to perfect the fine details of an image.
...
This is perhaps the most critical insight. The EDM authors introduced **network preconditioning**, wrapping the U-Net `F_θ` in scalers that are functions of the noise level `σ`:

D(x; σ) = c_skip(σ) * x + c_out(σ) * F_θ(c_in(σ) * x; σ)

* `c_in(σ)` scales the input, ensuring the network always sees data with a consistent variance.
* `c_out(σ)` scales the output, ensuring the network's prediction is correctly proportioned.
...
The EDM framework gives you more control, but with great power comes the question: how do you choose the parameters? Here’s a quick guide based on the new toolkit.
