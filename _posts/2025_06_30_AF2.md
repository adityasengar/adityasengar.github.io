...
![The triangular operations at the heart of the Evoformer. The core idea is that information about the relationship between residues (i, k) and (k, j) provides strong evidence about the 'closing' edge of the triangle, (i, j). As shown in panel (c), the model enforces this geometric consistency using two distinct mechanisms: a gated multiplicative update and a more selective, bias-driven self-attention step, which are applied to all possible triangles.](triangle_update.png)
*Figure 2. The triangular operations at the heart of the Evoformer.*

**Triangular Multiplicative Update: Densifying the Geometric Graph.**  

The first operation, the **Triangular Multiplicative Update**, densifies the geometric graph by ensuring local consistency. For each pair of residues $(i,j)$, it aggregates information from all possible triangular paths that pass through an intermediate residue $k$. The entire operation, which combines an update signal with a dynamic gate, can be expressed in a single, consolidated equation:

$$
z_{ij} \mathrel{+}= \underbrace{\sigma(W_g z_{ij})}_{\text{Gate}} \odot \underbrace{\sum_{k=1}^{N_{res}} \left( (W_a z_{ik}) \odot (W_b z_{jk}) \right)}_{\text{Update Signal from all Triangles}}
$$

Or, in plain text for clarity:


Here is what's happening in this substitution:

- On the left, $z_{ij} \mathrel{+}=$ signifies that we are updating our current representation for the residue pair $(i,j)$.
- The **Update Signal** on the right is the core of the triangle logic. It sums ($\sum_k$) evidence over all possible intermediate residues $k$. For each triangle, it takes the representations of the other two edges ($z_{ik}$ and $z_{jk}$), passes them through learned linear layers ($W_a, W_b$), and combines them. This creates a powerful signal that incorporates geometric evidence from the entire protein.
- The **Gate** is a learnable filter. It takes the current vector $z_{ij}$, passes it through its own linear layer ($W_g$) and a sigmoid function ($\sigma$), producing a vector of values between 0 and 1. This gate then multiplies ($\odot$) the *Update Signal*, effectively deciding how much of the new information to accept. A gate value near 0 ignores the update, while a value near 1 accepts it fully.

By summing over all possible $k$, this mechanism efficiently enforces local geometric agreement across the representation. The entire process is then repeated for "incoming" edges to ensure symmetry.
...
1. It first computes an **update vector** by combining information from the other two edges in the triangle. For the "outgoing" update, the logic is: 
   ...
   $$
   \text{update} = \sum_{k} \left( \text{Linear}(z_{ik}) \odot \text{Linear}(z_{jk}) \right)
   $$
