...
The leap from AlphaFold 2 to Boltz‑2 shows how fast a field can move once a breakthrough becomes truly open. Parts 1 and 2 tracked closed‑source models pushing the frontier of structure. Part 3 has shown the open community matching—and then surpassing—those gains by adding the things biologists value most: chemistry realism, user steering, and functional insight.

Boltz‑1’s MIT‑license release was the spark. Within months, researchers worldwide were tweaking kernels, enlarging crops, and docking ligands no closed server would touch. Boltz‑2 turned that spark into a bright torch, shifting the target from *shape* to *binding strength*. Questions that once required days of FEP can now be asked in seconds: “Will this mutation kill my drug?”

The lesson is broader. By blending gentle physics forces with data‑hungry deep nets, Boltz‑2 proves we don’t have to choose between black‑box AI and classical simulation; we can weave the two. Next‑gen models may fold in a lightweight molecular‑mechanics engine or learn the on/off rates that decide dosing schedules.

Viewed this way, Boltz‑2 is a **biomolecular foundation model**—a base network, trained on oceans of sequence, structure, and affinity data, ready to be fine‑tuned for anything from enzyme redesign to antibody escape. Its permissive licence invites spin‑offs: metal‑ion plugins, RNA‑fold hybrids, or community forks like **Chai‑1** [^chai1].

AlphaFold cracked the fold; OpenFold opened it; Boltz turned openness into traction—and traction into function. Each open generation seeds the next. We are moving from static snapshots to co‑designing dynamic molecular machines, with anyone free to join the work. That shared toolbox bodes well for faster disease cures and a deeper understanding of life’s chemistry.
