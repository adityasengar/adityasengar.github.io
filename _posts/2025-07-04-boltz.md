...
1. **A Universal Language (Tokens)**  
   AlphaFold 3 dropped its protein‑only mindset and adopted a token system that can describe any molecule.  
   * Standard biopolymers (proteins, DNA/RNA) → one token per residue.  
   * Diverse ligands → one token per heavy atom.  
   This lets the model reason over chemically varied complexes in a single pass.

2. **The Pairformer & Dethroning the MSA**  
   * **Pairformer replaces Evoformer.**  
   * MSA now gives a brief hint up front, then exits the stage; the remaining 48 cycles focus almost entirely on geometry.  
   * Triangle Attention still enforces global consistency, but the model relies less on evolutionary clues and more on raw chemistry and 3‑D logic.

3. **A Conditional Generative Core (Diffusion)**  
   The deterministic builder is gone. A diffusion “sculptor” starts from random noise and iteratively denoises it:  
   * Input: a cloud of atoms.  
   * Guidance: the single & pair representations from the Pairformer (the “conditional” blueprint).  
   * Output: a coherent, physically plausible structure—achieved step‑by‑step rather than in one shot.

Boltz builds directly on this blueprint, then layers on steering potentials and an affinity head to go from *fit* to *tightness*.
