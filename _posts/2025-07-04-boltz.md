...
Boltz‑Steering operates at **every** reverse‑diffusion step.  

First the network proposes a move for each atom; then the physics module scores that pose, produces a gentle “correction” vector, and the two motions are blended. Think of this correction as a guiding gradient—nudging atoms away from clashes or toward ideal bond angles—so the final update reflects both learned knowledge and hard chemistry rules.
