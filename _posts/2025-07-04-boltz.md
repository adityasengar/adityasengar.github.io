...
Figure 2. Align‑as‑you‑go keeps reverse diffusion stable.**  
(a)  Denoising network predicts a clean pose (stars) for noisy atoms xₜ (open circles); diamonds mark the half‑step target x̂ₜ′.  
(b)  **Vanilla AlphaFold 3** interpolates directly, producing long, divergent update vectors that twist the structure and accumulate rotational drift.  
(c)  **Boltz align‑as‑you‑go** first rigid‑aligns the prediction to xₜ, then interpolates, giving short, collinear moves and gentler geometry updates.  
Adapted from Passaro et al. 2025 (CC‑BY 4.0). See “Diffusion Pipeline” for discussion.
