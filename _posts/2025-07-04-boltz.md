...
In the first part of this series, we saw how AlphaFold 2 conquered a 50-year-old grand challenge [blog_af2]. In the second [blog_af3], we watched AlphaFold 3 take a generative leap, moving from single proteins to entire molecular symphonies. These were monumental achievements, but they came with a catch: the most advanced model, AlphaFold 3, remained behind a closed door, limiting its use for widespread research and development.

In late 2024, a team at MIT made a bold move by releasing **Boltz-1**, the first fully open-source model approaching the prowess of AlphaFold 3. This was more than just a reimplementation; it was a statement. AlphaFold 3 had demonstrated that *generative diffusion* models could tackle multi-molecule structure prediction, but its closed license limited widespread use. Boltz-1 shattered that barrier, quickly becoming one of the most adopted *co-folding* models in industry. Within weeks, companies like Recursion were reportedly integrating Boltz-1 into their drug discovery pipelines, proving the immense demand for accessible, state-of-the-art AI.

But Boltz-1 was just the opening act. In mid-2025, its successor, Boltz-2, arrived and changed the game entirely. It pushed beyond just predicting structures and into the realm of predicting function. The name "Boltz" itself is a nod to Boltzmann and the physics of thermodynamics—a fitting choice for a model that not only asks "how do these molecules fit?" but also "how tightly will they hold on?" **Boltz-2** is heralded as the first AI model to approach the accuracy of rigorous free-energy perturbation (FEP) simulations while operating over 1000x faster.[^1] In practical terms, what used to take 6–12 hours of computation can now be achieved in seconds on a single GPU. This leap enables researchers to screen thousands of drug candidates virtually and rapidly zero in on the most promising ones — an advancement poised to cut down drug development timelines.
...
* **Architectural Foundations and Efficiency:** How Boltz-1 replicated and optimized the AlphaFold 3 architecture, introducing tricks like *TriFast* attention kernels to enable larger complexes and faster inference.
...
![Architectural comparison of Boltz vs AlphaFold 3.](Boltz_AF3.png)
*<p align="center"><b>Architectural comparison of Boltz vs AlphaFold 3.</b> Boltz-2’s trunk is deeper (64 layers vs 48) and uses optimized kernels, enabling it to process larger complexes with similar or less computational effort than AlphaFold 3.</p>*

At its core, Boltz-1 adopted the powerful blueprint pioneered by AlphaFold 3 (see Part 2 [blog_af3]): a deep learning system composed of a **Pairformer** trunk to learn rich representations of the entire molecular system, and a **diffusion-based generator** to translate those representations into a 3D structure. The guiding philosophy was clear: match the performance of the closed-source model, but make it faster, more efficient, and accessible to all.
...
![Visualizing one reverse diffusion step with and without alignment.](Boltz_AF_reverse_diff.png)
*<p align="center"><b>Visualizing one reverse diffusion step with and without alignment.</b> (<i>Left</i>) The model starts from a noisy input (open circles) and predicts a denoised structure (stars). If the prediction has a slight rigid-body rotation relative to the input, directly interpolating (center) yields long update vectors (arrows) that drag atoms in possibly unphysical ways. Boltz’s method (right) first aligns the predicted structure to the input, then interpolates, resulting in much shorter, more collinear movements that correspond to gentle adjustments of internal geometry.</p>*
...
1.  **[blog_af2]** Sengar, A. *Deconstructing the Fold with AlphaFold 2*. Blog Post (Part 1 of this series), 2025.
2.  **[blog_af3]** Sengar, A. *Denoising the Complex with AlphaFold 3*. Blog Post (Part 2 of this series), 2025.
3.  **[mit_boltz1]** MIT Schwarzman College of Computing (2024). *MIT researchers introduce Boltz-1, a fully open-source model for predicting biomolecular structures*. (News release) 【Retrieved from news.mit.edu】
4.  **[github_boltz]** Wohlwend, J. (2024). *Official repository for the Boltz biomolecular interaction models*. GitHub. 【Online】 Available: [https://github.com/jwohlwend/boltz](https://github.com/jwohlwend/boltz)
5.  **[bioit_boltz2]** Bio-IT World (2025, June 6). *MIT Researchers Unveil Boltz-2: AI Model Predicts Protein Structure, Binding Affinity in Seconds*.
6.  **[rowan_faq]** Rowan (2025). *The Boltz-2 FAQ*. (Web article) 【Retrieved from rowansci.com】
7.  **[nature_af3]** Abramson, J., Adler, J., Dunger, J., *et al*. (2024). Accurate structure prediction of biomolecular interactions with AlphaFold 3. *Nature*, 630:493–500.
8.  **[genengnews_boltz2]** Genetic Engineering & Biotechnology News (2025, June). *Boltz-2 Released to Democratize AI Molecular Modeling for Drug Discovery*.
9.  **[biorxiv_boltz1]** Wohlwend, J., Corso, G., *et al*. (2024). *Boltz-1: Democratizing Biomolecular Interaction Modeling*. bioRxiv preprint **2024.11.19.624167**.
10. **[wohlwend_boltz2]** Passaro, S., Corso, G., Wohlwend, J., *et al*. (2025). *Boltz-2: Towards Accurate and Efficient Binding Affinity Prediction*. bioRxiv preprint **2025.06.14.659707**.
11. **[dpeptide_af3]** Choudhury, A., Deo, C., Dowdell, B.J., *et al*. (2025). Has AlphaFold 3 solved the protein folding problem for D-peptides? *bioRxiv* **2025.03.10.** (Preprint).

***

[^1]: FEP is a cornerstone of computational chemistry, providing highly accurate predictions of binding affinity by rigorously simulating the physical transformation of one molecule into another. This accuracy, however, comes at an immense computational cost, which has historically limited its use to only a handful of the most promising drug candidates.
