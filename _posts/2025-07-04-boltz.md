...
> **Predicting the pose isn’t the finish line—knowing whether the pose *sticks* is.**  
> In early 2016, a patient with ALK‑positive lung cancer relapsed on the third‑generation inhibitor **lorlatinib**. Sequencing revealed a single pocket change, **L1198F**. X‑ray data showed the drug still nestled in the kinase groove, yet its potency had collapsed. Binding assays traced the failure to a sharp drop in affinity, not a docking error. Paradoxically, the same mutation *tightened* the grip of the older drug **crizotinib**, giving the patient another treatment window. :contentReference[oaicite:0]{index=0}  
> This episode highlights a blind spot in structure‑only predictors: correct geometry can coexist with poor binding strength.
...
In late 2024, a team at MIT made a bold move by releasing **Boltz-1**[[^boltz1]], the first fully open-source model approaching the prowess of AlphaFold 3[^licensing-status]. This was more than just a reimplementation; it was a statement. AlphaFold 3 had demonstrated that *generative diffusion* models could tackle multi-molecule structure prediction, but its closed license at that time limited widespread use. Boltz-1 shattered that barrier, quickly becoming one of the most adopted *co-folding* models in industry. Within weeks, companies like Recursion were reportedly integrating Boltz-1 into their drug discovery pipelines, proving the immense demand for accessible, state-of-the-art AI.
...
This post will dissect the Boltz-1 and Boltz-2 models, framing them not as mere clones, but as the next chapter in the protein prediction revolution—a chapter written by and for the open-source community. We'll explore how they refined the AlphaFold 3 blueprint and then forged into new territory by tackling the holy grail of drug discovery: binding affinity.




<details>
<summary><strong>🧠 Refresher: Key Concepts from AlphaFold 3</strong></summary>
<div style="border: 1px solid #ddd; border-radius: 5px; background: #f9f9f9; padding: 1em; margin-top: 1em;">
To understand how Boltz builds upon AlphaFold 3, let's recall the two revolutionary shifts we covered in Part 2:
<ul>
  <li><strong>A Universal Language:</strong> AF3 moved beyond just proteins, using a "token" system to represent any molecule (proteins, DNA, RNA, ligands), allowing it to model entire complexes.</li>
  <li><strong>Generative Diffusion:</strong> It replaced a deterministic builder with a generative "sculptor" that starts with a random cloud of atoms and "denoises" it into a final structure, guided by a learned blueprint.</li>
</ul>
Boltz adopts this powerful generative blueprint as its starting point.
</div>
</details>
...
### Training a Titan: Smart Shortcuts for a Massive Model
Training a model as massive as Boltz-1 was a monumental undertaking, requiring not just immense compute power but also clever strategies to learn from biological data too large for any single GPU. To solve this, the team developed a "crop-and-recycle" technique, akin to learning a giant jigsaw puzzle by studying smaller, overlapping sections. To give the model a well-rounded training diet, this was done in two ways: 

**Spatial Cropping** focused on the intricate 3D details of a binding site, while 
**Sequence Cropping** taught the model about broader, connected structural elements. This was paired with other pragmatic features, like "robust pocket conditioning," which allows researchers to guide the model by specifying a known binding site. These smart shortcuts were essential, transforming Boltz-1 from a theoretical architecture into a flexible and efficient research partner.
